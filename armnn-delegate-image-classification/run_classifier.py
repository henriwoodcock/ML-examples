...
def check_args(args):
  ...
  if not input_image_p.suffix in (".png", ".jpg", ".jpeg"):
    raise IOError("--input_image option should point to an image file of the "\
    "format .jpg, .jpeg, .png")
  ...
  if not all([backend in ["GpuAcc", "CpuAcc", "CpuRef"] for backend in args.preferred_backends]):
    raise ValueError("Incorrect backends given. Please choose from 'GpuAcc',"\
      " 'CpuAcc', 'CpuRef'.")

  return None

def load_image(image_path: Path, model_input_dims: Union[tuple, list],
  grayscale: bool):
...
def load_delegate(delegate_path: Path, backends: list):
  ...
  #load delegate
  armnn_delegate = tflite.load_delegate(library=delegate_path.as_posix(),
...
def load_tf_model(model_path: Path, armnn_delegate: tflite.Delegate):
  ...
  """
  interpreter = tflite.Interpreter(model_path=model_path.as_posix(),
...
def format_output(output_data, label_mapping):
...
def main(args):
  ...
  output_prediction = format_output(output_tensor, labelmapping)
